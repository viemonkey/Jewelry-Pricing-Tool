...
> [!IMPORTANT]
> Hệ thống sử dụng **Tailwind CSS v4** + **Shadcn UI** + **Radix UI**.
> Mọi styling đều dùng Tailwind utility classes. **KHÔNG viết CSS thuần** (ngoại trừ `:root` variables trong `src/index.css`).

### Kiến Trúc Thư Mục Thành Phần UI
```
src/components/
├── ui/                  ← Shadcn UI chuẩn (do shadcn CLI sinh ra, KHÔNG sửa tay)
│   ├── button.tsx
│   ├── input.tsx
│   ├── badge.tsx
│   ├── card.tsx
...
│   ├── accordion.tsx
│   └── textarea.tsx
├── custom-ui/           ← Component tuỳ chỉnh riêng (Có thể sửa trực tiếp)
│   ├── calendar.tsx     ← Calendar dựa trên react-day-picker
│   ├── select.tsx       ← Select dựa trên Radix UI, styled Tailwind
│   └── date-picker.tsx  ← DatePicker kết hợp Calendar + Popover
├── shared/              ← Component chia sẻ nội bộ
│   └── ProgressStar/
...
└── MainLayout.jsx       ← Layout chính (Sidebar + Header + Content)
```

> [!NOTE]
> - `src/components/ui/` — Shadcn chuẩn. Thêm component mới dùng `npx shadcn@latest add <tên>`.
> - `src/components/custom-ui/` — Component tự tuỳ chỉnh. Được phép sửa trực tiếp.
> - **Tuyệt đối không viết CSS thuần hay class tự chế ngoài Tailwind.**
...
| `5` (`1.25rem`) | `20px` | Padding chính của Layout (Wrapper trong MainLayout.jsx: `p-4 sm:p-5`), khoảng cách giữa các phần lớn |
...
| `z-header` | `100` | Thanh tiêu đề trên (`Header.jsx`, class `h-[52px]`) |
| `z-sidebar`| `100` | Thanh điều hướng trái (`Sidebar.jsx`, class `w-[210px]`) |
...
> [!NOTE]
> Các component đánh dấu ✅ đã cài đặt. Các component ⚙️ cần chạy `npx shadcn@latest add <tên>` trước khi dùng.

| Component | Import Statement & File Path | Mục đích | Khi nào dùng | Khi nào KHÔNG dùng |
| :--- | :--- | :--- | :--- | :--- |
| ✅ **Button** | `import { Button } from "@/components/ui/button"` | Kích hoạt hành động | Gửi form, mở hộp thoại, thực hiện tác vụ | Thay thế liên kết văn bản đơn giản |
| ✅ **Input** | `import { Input } from "@/components/ui/input"` | Nhập văn bản 1 dòng | Tên, Email, SĐT, Mã định danh | Nhập mô tả dài nhiều dòng |
| ✅ **Textarea** | `import { Textarea } from "@/components/ui/textarea"` | Nhập văn bản nhiều dòng | Phản hồi, mô tả sản phẩm, ghi chú | Nhập dữ liệu ngắn có định dạng |
| ✅ **Select** *(custom)* | `import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/custom-ui/select"` | Chọn 1 giá trị từ danh sách | Danh sách cố định `< 10` mục | Danh sách `> 10` mục hoặc có tìm kiếm |
| ✅ **Calendar** *(custom)* | `import { Calendar } from "@/components/custom-ui/calendar"` | Hiển thị lịch chọn ngày | Chọn ngày/tháng/năm trực quan | Nhập năm hoặc tháng đơn thuần |
| ✅ **DatePicker** *(custom)* | `import { DatePicker } from "@/components/custom-ui/date-picker"` | Chọn ngày bằng Popover | Ngày sinh, lọc khoảng thời gian báo cáo | Nhập năm hoặc tháng đơn thuần |
| ✅ **Badge** | `import { Badge } from "@/components/ui/badge"` | Nhãn trạng thái phân loại | Trạng thái đơn hàng (Đang xử lý, Thất bại) | Làm nút bấm kích hoạt hành động |
| ✅ **Card** | `import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card"` | Bao bọc nhóm thông tin | Widget Dashboard, khối thông tin liên hệ | Hiển thị dữ liệu dạng bảng lặp lại |
| ✅ **DropdownMenu** | `import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem } from "@/components/ui/dropdown-menu"` | Menu hành động ngữ cảnh | Danh sách thao tác (Sửa, Xóa, Xuất dữ liệu) | Làm thanh menu điều hướng chính |
| ✅ **Popover** | `import { Popover, PopoverTrigger, PopoverContent } from "@/components/ui/popover"` | Popup nhỏ khi click | Date picker, filter panel nhỏ | Thông báo toàn màn hình |
| ✅ **Accordion** | `import { Accordion, AccordionItem, AccordionTrigger, AccordionContent } from "@/components/ui/accordion"` | Mở rộng / thu gọn nội dung | FAQ, cài đặt theo nhóm | Điều hướng trang chính |
| ✅ **Table** | `import { Table, TableHeader, TableBody, TableRow, TableHead, TableCell } from "@/components/ui/table"` | Hiển thị dữ liệu dạng bảng | Danh sách khách hàng, lịch sử giao dịch | Hiển thị dữ liệu dưới 3 bản ghi đơn giản |
| ⚙️ **Dialog** | `npx shadcn@latest add dialog` → `import { Dialog } from "@/components/ui/dialog"` | Hộp thoại xác nhận | Xác nhận xóa dữ liệu, Form thêm nhanh | Form đăng ký/quy trình dài nhiều bước |
| ⚙️ **Sheet** | `npx shadcn@latest add sheet` → `import { Sheet } from "@/components/ui/sheet"` | Slide-out panel chi tiết | Xem chi tiết Khách hàng/Sản phẩm | Hiển thị thông báo ngắn gọn |
| ⚙️ **Tabs** | `npx shadcn@latest add tabs` → `import { Tabs } from "@/components/ui/tabs"` | Phân nhóm nội dung cùng cấp | Chuyển đổi giữa các nhóm thông tin | Điều hướng sang trang khác nhau |
| ⚙️ **Tooltip** | `npx shadcn@latest add tooltip` → `import { Tooltip } from "@/components/ui/tooltip"` | Giải thích khi hover | Icon-only button | Thông tin quan trọng bắt buộc phải đọc |
| ⚙️ **Skeleton** | `npx shadcn@latest add skeleton` → `import { Skeleton } from "@/components/ui/skeleton"` | Trạng thái chờ tải | Placeholder khi gọi API | Hành động tương tác đồng bộ tức thì |
| ⚙️ **Toast** | `npx shadcn@latest add toast` → `import { toast } from "sonner"` | Thông báo không chặn | "Đã cập nhật", "Lỗi kết nối" | Yêu cầu người dùng lựa chọn Có/Không |
...
- **Error Text (Thông báo lỗi):** `text-xs text-danger-600 mt-1 block animate-in fade-in duration-150`
- **Focus State:** `focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`

### Ví dụ cấu trúc mã React mẫu cho Form Field (sử dụng Shadcn Input):
```tsx
...
- **Thư viện chính:** **Phosphor Icons** (`@phosphor-icons/react`) — thư viện icon chính của dự án.
- **Thư viện phụ:** **Lucide Icons** — được Shadcn UI dùng nội bộ cho một số component (ví dụ: Check, ChevronDown trong Select).

```tsx
// Dùng Phosphor Icons (ưu tiên)
import { Globe, CheckCircle, Warning, Sparkle } from "@phosphor-icons/react"

// Chỉ dùng Lucide khi cần thiết (Shadcn built-in)
import { ChevronDown, Check } from "lucide-react"
```

- **Kích thước tiêu chuẩn:**
  - Nhỏ (inline text, button nhỏ): `size={14}`
  - Mặc định (thanh menu, button thường, danh sách): `size={16}`
  - Lớn (Dashboard KPI, Empty State, Header): `size={24}` hoặc `size={32}`
- **Weight mặc định:** Dùng weight `regular` (mặc định) hoặc `bold` when cần nhấn mạnh.
...
- **Animations phổ biến:**
  - `animate-in fade-in duration-150` (fadeIn): Mở mờ dần cho tooltip và popup nhỏ.
  - `animate-in fade-in slide-in-from-bottom-3 duration-200` (slideUp): Trượt nhẹ từ dưới lên (12px) kết hợp mờ dần cho Dialog/Modal khi mở ra.
...
9.  **Bọc trang chuẩn Layout:** Các trang mới (pages) được tự động render qua `<Outlet />` bên trong `MainLayout.jsx`. Thẻ ngoài cùng của trang con nên dùng `<div className="flex flex-col gap-6">` (hoặc các lớp flex tương tự) để căn chỉnh bố cục đồng nhất.
